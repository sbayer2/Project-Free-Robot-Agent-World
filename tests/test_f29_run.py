...
def _gates(gain_min=4.0, jpr=30.0, dpr=28.0, r=0.99):
...
def test_arm_record_schema_feeds_the_frozen_verdicts_script(tmp_path):
    ...
    arms = {t: arm_record(rs[t], _gates(r=rs[t]), joint[t], disjoint[t])
            for t in ARMS}
...
def test_failed_gate_withholds_verdicts(tmp_path):
    ...
    for t in ARMS:
        g = _gates(r=rs[t])
        if t == "g2":
            g["disjoint_train_gain_min"] = 1.2  # below RETRAIN_SANITY 1.5
...
def test_missing_amendment_fails_pilot_gate(tmp_path):
    ...
    rs = {"ctrl": 0.008, "base": 0.263, "g2": 0.809, "loud": 0.992}
    arms = {t: arm_record(rs[t], _gates(r=rs[t]), [2.0, 2.1, 1.9],
